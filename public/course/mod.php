...
$returnpagelevel = optional_param('returnpagelevel', null, PARAM_INT);
...
if ($returnpagelevel < 0) {
    $returnpagelevel = null;
}
...
if (!is_null($returnpagelevel)) {
    $url->param('returnpagelevel', $returnpagelevel);
    $returnoptions['pagelevel'] = $returnpagelevel;
}
...
if (!empty($add)) {
    ...
    redirect(
        new moodle_url(
            '/course/modedit.php',
            array_merge($params, $returnparams),
        )
    );
    ...
    if (!$confirm or !confirm_sesskey()) {
        ...
        $formcontinue = new single_button(
            new moodle_url("$CFG->wwwroot/course/mod.php", array_merge($optionsyes, $returnparams)),
            get_string('yes')
        );
...
if ((!empty($movetosection) or !empty($moveto)) and confirm_sesskey()) {
    ...
    unset($USER->activitycopyname);
    unset($USER->activitycopysectionreturn);
    redirect(course_get_url($course, $cm->sectionnum, ['navigation' => null]));
} else {
