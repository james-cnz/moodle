    ...
    public function export_for_template(\renderer_base $output): stdClass {
        ...
        while ($back >= 0 && empty($data->previousurl)) {
            if ($canviewhidden || $sections[$back]->uservisible) {
                ...
                $data->previousurl = course_get_url($course, $back, ['navigation' => true, 'pagelevel' => PAGE_LEVEL_SPECIFIED]);
        ...
        while ($forward <= $numsections and empty($data->nexturl)) {
            if ($canviewhidden || $sections[$forward]->uservisible) {
                ...
                $data->nexturl = course_get_url($course, $forward, ['navigation' => true, 'pagelevel' => PAGE_LEVEL_SPECIFIED]);
                // If there is no url for the section the link should not be displayed.
